...
const RANGE_PRESETS = {
  ...
  '90d': { label: 'Last 90 days', durationMs: 90 * 24 * 60 * 60 * 1000, bucket: 'week', minBuckets: 10 },
  all: { label: 'All time', durationMs: null, bucket: 'month', minBuckets: 12 },
};
...
async function fetchModelsForScope(pool, organizationId, accessibleTeamIds, role) {
  ...
  const { rows } = await pool.query(
    `
      ...
      LEFT JOIN providers p ON p.id = m.provider_id
      WHERE m.organization_id = $1
        AND (m.team_id IS NULL OR m.team_id = ANY($2::uuid[]))
      ORDER BY name ASC
    `,
    params,
  );
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const pool = getPool();
    const { organizationId, teamId: sessionTeamId } = await resolveActiveContext(pool, session);
