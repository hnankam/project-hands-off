 ...
 * the OAuth flow using the Better Auth client.
 */
...
const PROVIDER_INFO: Record<SocialProvider, { name: string; icon: React.ReactNode }> = {
  google: {
    name: 'Google',
    icon: (
...
export default function OAuthPage({ provider, onSuccess, onError }: OAuthPageProps) {
  ...
  const providerInfo = PROVIDER_INFO[provider] || { name: provider, icon: null };
  ...
  useEffect(() => {
    const startOAuth = async () => {
      try {
        ...
        const result = await signInWithSocialDirect(provider);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {status === 'loading'
            ? `Please wait while we redirect you to ${providerInfo.name}.`
            : `Unable to connect to ${providerInfo.name}.`}
        </p>
