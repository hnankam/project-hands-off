...
export const HomePage: React.FC<HomePageProps> = ({ isLight, onGoToSessions, onGoAdmin }) => {
  ...
  const totalSessionPages = useMemo(
    () => Math.ceil(sortedSessions.length / SESSIONS_PER_PAGE),
  ...
  const averageTokensPerTrackedSession =
    sessionsWithUsageCount > 0 ? aggregatedUsage.total / sessionsWithUsageCount : 0;
  const averageTokensPerRequest =
    aggregatedUsage.requestCount > 0 ? aggregatedUsage.total / aggregatedUsage.requestCount : 0;

  const promptShare =
    aggregatedUsage.total > 0
      ? Math.min(100, Math.max(0, Math.round((aggregatedUsage.request / aggregatedUsage.total) * 100)))
      : 0;
  const completionShare = aggregatedUsage.total > 0 ? Math.min(100, Math.max(0, 100 - promptShare)) : 0;
  ...
  const renderIcon = useCallback(
    (icon: 'activity' | 'sparkles' | 'stack' | 'team') => {
      ...
      switch (icon) {
        ...
        case 'sparkles':
          return (
            <svg className={baseClasses} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
              <path
                d="M12 3v3m0 12v3m9-9h-3M6 12H3m15.364 6.364l-2.121-2.121M6.757 6.757 4.636 4.636m0 14.728 2.121-2.121m12.728-12.728-2.121 2.121"
                strokeLinecap="round"
  ...
  const statusToneClasses = useCallback(
    (tone: 'success' | 'info' | 'muted') => {
      switch (tone) {
        case 'success':
          return isLight ? 'bg-emerald-100 text-emerald-700' : 'bg-emerald-900/20 text-emerald-300';
        case 'info':
          return isLight ? 'bg-blue-100 text-blue-700' : 'bg-blue-900/20 text-blue-300';
        default:
          return isLight ? 'bg-gray-100 text-gray-600' : 'bg-gray-800/50 text-gray-400';
      }
    },
    [isLight],
  );
  ...
  return (
    <div className="home-page flex min-h-0 flex-1 flex-col">
      {/* Home Page Header */}
      <div
        className={cn(
          'relative z-20 flex h-[37px] min-h-[37px] flex-shrink-0 items-center justify-between border-b px-2',
        ...
        <div className="flex items-center gap-1">
          ...
          <UserMenu isLight={isLight} onGoAdmin={tab => onGoAdmin?.(tab)} onGoToSessions={onGoToSessions} />
        </div>
      </div>

      {/* Floating Alerts - same banner style as admin pages */}
      {(success.visible || error.visible) && (
        <div className="pointer-events-none fixed top-[37px] right-0 left-0 z-[10002] px-4 pt-4">
      ...
      {/* Home Page Content */}
      <div className={cn('relative z-10 min-h-0 flex-1 overflow-hidden', isLight ? 'bg-white' : 'bg-[#0D1117]')}>
        <div className="home-page-scroll h-full min-h-0 overflow-y-auto">
          <div className="space-y-4 px-3 py-4">
            {/* Hero */}
            <div
              className={cn(
                'relative mx-auto max-w-4xl overflow-hidden rounded-xl border px-4 py-5',
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 text-[11px]">
                    <span
                      className={cn(
                        ...
                        isLight ? 'bg-gray-100 text-gray-600' : 'bg-gray-800/40 text-gray-300',
                      )}>
                    ...
                    <span
                      className={cn(
                        'inline-flex items-center gap-1 rounded-md px-2 py-1',
                        isLight ? 'bg-gray-100 text-gray-500' : 'bg-gray-800/40 text-gray-400',
                      )}>
                      <svg
                        className="h-3.5 w-3.5"
                        viewBox="0 0 24 24"
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button
                    ...
                    className={cn(
                      'inline-flex items-center justify-center gap-1.5 rounded-md border px-3 py-3 text-xs font-semibold transition-colors',
                      creatingSession
                        ? isLight
                          ? 'cursor-wait border-gray-300 bg-gray-200 text-gray-400'
                          : 'cursor-wait border-gray-600 bg-gray-700 text-gray-400'
                        : !canAccessSessions
                          ? isLight
                            ? 'cursor-not-allowed border-gray-300 bg-gray-200 text-gray-500'
                            : 'cursor-not-allowed border-gray-700 bg-gray-800 text-gray-500'
                          : isLight
                            ? 'border-blue-300/70 bg-white text-gray-600 hover:bg-gray-50'
                            : 'border-blue-700/70 bg-[#151C24] text-gray-300 hover:bg-[#1a2330]',
                    )}>
                    {creatingSession ? (
                      <svg
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        isLight
                          ...
                          : 'border-gray-700 bg-[#151C24] text-gray-300 hover:bg-[#1a2330]',
                      )}>
                      <svg
                        className="h-3.5 w-3.5"
                        viewBox="0 0 24 24"
            ...
            <div ref={sentinelRef} className="-mb-[1px] h-0" />

            {/* Tab Bar - Sticky */}
            <div
              ref={tabBarRef}
              className={cn(
                'sticky top-0 z-10 mx-auto flex h-[37px] max-w-4xl items-center justify-center gap-2 px-2 py-1 transition-all duration-200',
                isLight ? 'bg-gray-50' : 'bg-[#151C24]',
                isTabBarSticky
                  ? cn('rounded-b-lg border-x border-b', isLight ? 'border-gray-200' : 'border-gray-700')
                  : 'rounded-lg',
              )}>
              <div className="flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {statCards.map(card => (
                    <div
                      ...
                      <div className={cn('text-2xl font-semibold', mainTextColor)}>{card.value}</div>
                ...
                <div
                  ...
                  <div className={cn('border-b', isLight ? 'border-gray-200' : 'border-gray-700')}>
                    ...
                    <div className="flex items-center justify-between px-4 py-2">
                      <h3 className={cn('text-sm font-semibold', mainTextColor)}>Recent Chats</h3>
                      ...
                      <div
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className={cn('px-2 text-xs', isLight ? 'text-gray-700' : 'text-gray-300')}>
                  ...
                  {sessionsLoading ? (
                      ...
                      <div className="animate-pulse space-y-2">
                        {Array.from({ length: 4 }).map((_, idx) => (
                          <div
                            ...
                            className="h-10 rounded-md bg-gray-200/70 dark:bg-gray-800/70"
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <div
                      className="recent-sessions-scroll max-h-80 w-full overflow-auto"
                      style={{ '--table-scroll-bg': isLight ? '#FFFFFF' : '#151C24' } as React.CSSProperties}>
                      <table
                        className={cn(
                          'w-full min-w-full border-collapse divide-y text-xs',
                          ...
                          <tr>
                            {bulkSelectMode && (
                              <th
                                ...
                                <div className="flex cursor-pointer items-center gap-2" onClick={toggleSelectAll}>
                                  <div
                                    className={cn(
                                      'flex h-3.5 w-3.5 flex-shrink-0 items-center justify-center rounded transition-opacity',
                                      paginatedSessions.length > 0 &&
                                        paginatedSessions.every(s => selectedSessions.has(s.id))
                          ...
                          {paginatedSessions.map(session => {
                            ...
                            return (
                              <tr
                                ...
                                <td className={cn('max-w-[200px] truncate px-3 py-2')}>
                                  <div className="flex items-center gap-2">
                                    {renamingSessionId === session.id ? (
                                      <div className="flex min-w-0 flex-1 items-center gap-2">
                                        <input
                                          ...
                                          className={cn(
                                            'min-w-0 flex-1 rounded border px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-blue-500',
                                            isLight
                                ...
                                <td className={cn('max-w-[150px] truncate px-3 py-2', mainTextColor)}>
                                  {agentIdToName[session.selectedAgent] || prettifyLabel(session.selectedAgent)}
                                </td>
                                <td
                                  className={cn(
                                    'max-w-[150px] truncate px-3 py-2',
            ...
            {activeHomeTab === 'usage' && (
              <div className="animate-fadeIn mx-auto max-w-4xl space-y-4">
                ...
                <div
                  ...
                  <div className={cn('relative border-b px-4 py-3', isLight ? 'border-gray-200' : 'border-gray-700')}>
                    <div className="pr-32">
                      <h3 className={cn('text-sm font-semibold', mainTextColor)}>Usage Overview</h3>
                    ...
                    <div className="absolute top-3 right-4 flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={cn(
                          'flex h-8 w-8 flex-shrink-0 items-center justify-center rounded border transition-colors',
                          isLight
      ...
      <div
        ...
        <div className="flex items-center justify-between px-4 py-1.5">
          <div className={cn('text-xs font-medium', isLight ? 'text-gray-700' : 'text-gray-300')}>v {version}</div>
          <div className="flex items-center gap-1">
      ...
      {settingsOpen && (
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: Z_INDEX.modal }}>
            <div
              className={cn('w-full max-w-sm rounded-lg shadow-xl', isLight ? 'bg-white' : 'bg-[#151C24]')}
              onClick={e => e.stopPropagation()}>
              {/* Header */}
              <div
                className={cn(
      ...
      <AdminConfirmDialog
        ...
        message={
          <span>
            Permanently delete <strong>{selectedSessions.size}</strong> {selectedSessions.size === 1 ? 'chat' : 'chats'}
            ?<br />
            <br />
