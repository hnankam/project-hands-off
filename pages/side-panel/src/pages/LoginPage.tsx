...
import { requestPasswordReset, resetPasswordWithToken } from '../lib/auth-client';
...
export default function LoginPage({ onGoToInvitation }: LoginPageProps = {}) {
  ...
  const handleModeChange = (mode: 'sign-in' | 'sign-up' | 'forgot-password' | 'enter-token') => {
    setIsSignUp(mode === 'sign-up');
    setIsForgotPassword(mode === 'forgot-password');
    setIsEnterToken(mode === 'enter-token');
    setError(null);
    setSuccessMessage(null);
        ...
        <div className="mx-auto flex min-h-full max-w-6xl items-start justify-center px-4 pt-24 pb-10 sm:pt-24 sm:pb-16">
          <div
            ...
            <div className="relative px-7 pb-7 pt-9 sm:px-8 sm:pb-8 sm:pt-10">
              ...
              {/* Sign-in / Sign-up Toggle - Hidden during forgot password and enter token */}
              {!isForgotPassword && !isEnterToken && (
              <div
                className={cn(
              ...
              {/* Invitation-Only Banner - Hidden during forgot password and enter token */}
              {INVITATION_ONLY && !isForgotPassword && !isEnterToken && (
                <div
                  className={cn(
              ...
              {/* Enter Token Form - For pasting reset token */}
              {isEnterToken ? (
                <form onSubmit={handleResetWithToken} className="mt-5 space-y-3" noValidate>
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      className={cn(
                        'w-full px-2.5 py-1.5 text-xs border rounded-md outline-none focus:ring-1 focus:ring-blue-500 transition-colors font-mono resize-none',
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={cn(
                          'w-full py-1.5 px-2.5 pr-9 text-sm border rounded-md outline-none focus:ring-1 focus:ring-blue-500 transition-colors',
                          isLight ? 'bg-white border-gray-300 text-gray-900 placeholder:text-gray-400' : 'bg-[#151C24] border-gray-600 text-white placeholder:text-gray-500',
                        )}
                  ...
                  <div className="pt-2">
                    <button
                      type="submit"
                      disabled={isLoading || !resetToken || !password || !confirmPassword || password !== confirmPassword}
                      className={cn(
                        ...
                        (isLoading || !resetToken || !password || !confirmPassword || password !== confirmPassword) && 'cursor-not-allowed opacity-70',
                      )}>
                      {isLoading ? (
                  ...
                  <div className="text-center pt-2">
                    <button
                      ...
                      onClick={() => handleModeChange('sign-in')}
                      className={cn(
                        'text-xs font-medium underline-offset-4 transition-colors hover:underline',
                ...
                <form onSubmit={handleForgotPasswordSubmit} className="mt-5 space-y-3" noValidate>
                  <div className="space-y-1">
                    <label
                      htmlFor="email"
                      className={cn(
                  ...
                  <div className="text-center pt-2 space-y-2">
                    <button
                      type="button"
                      onClick={() => handleModeChange('enter-token')}
                      className={cn(
                        'text-xs font-medium underline-offset-4 transition-colors hover:underline block w-full',
