...
const AutoScrollDiv: FC<AutoScrollDivProps> = memo(
  ({ content, isStreaming, className = '', style, children, threshold = 50 }) => {
...
export const FileManagementCard: React.FC<FileManagementCardProps> = memo(
  ({
    ...
    const chatPreview = useChatPreviewOptional();
    ...
    const handleOpenInPreview = useCallback(
      async (e: React.MouseEvent) => {
        ...
        let body =
          ...
          fetchedContent ||
          content ||
          (validationError?.partial_data && typeof validationError.partial_data.content === 'string'
            ? validationError.partial_data.content
            : '') ||
          '';

        const isPlaceholder =
          body.startsWith('// Loading file content') || body.startsWith('// Failed to load file content');

        if ((!body || isPlaceholder) && fileId) {
          ...
          try {
            ...
            console.error('[FileManagementCard] Preview fetch failed:', err);
            return;
          } finally {
        ...
        const derivedFromFolder = folder ? `${folder}/${fileName}` : fileName;
        let resolvedPath = derivedFromFolder;
        const rawPath = fileInfo.file_path;
    ...
    return (
      <div
        ...
        {/* Header - Accordion row + optional Preview open */}
        <div
          style={{
            width: '100%',
            display: 'flex',
          ...
          {isComplete && !hasError && chatPreview && (
            <button
              type="button"
              onClick={handleOpenInPreview}
              disabled={isOpeningPreview}
              title="Expand to preview panel"
              aria-label="Expand to preview panel"
              className="flex flex-shrink-0 items-center justify-center self-center rounded-md"
              style={{
                width: 28,
                height: 28,
                ...
                padding: 0,
                border: 'none',
                cursor: isOpeningPreview ? 'wait' : 'pointer',
                backgroundColor: 'transparent',
                color: mutedTextColor,
                opacity: isOpeningPreview ? 0.5 : 1,
              }}
              onMouseEnter={e => {
                if (!isOpeningPreview) {
                  e.currentTarget.style.backgroundColor = isLight ? 'rgba(0, 0, 0, 0.06)' : 'rgba(255, 255, 255, 0.08)';
