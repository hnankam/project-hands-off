...
export const StatusBar: FC<StatusBarProps> = memo(
  ({
    ...
    return (
      <div
        className={`flex h-[35px] min-h-[35px] items-center justify-between gap-2 border-b px-2 py-0 ${
        ...
        <div
          className={`mr-4 flex min-w-0 flex-1 items-center gap-3 text-xs ${
            isLight ? 'text-gray-600' : 'text-gray-400'
          }`}>
          {/* Message counters and usage stacked */}
          <div className="-my-0.5 flex flex-shrink-0 flex-col items-end gap-0 text-[10px] leading-[1.1]">
          ...
          {/* Separator after stats (shown on web and extension) */}
          <div className={`h-6 w-px flex-shrink-0 ${isLight ? 'bg-gray-300' : 'bg-gray-600'}`} />

          {isExtensionContext() ? (
            <div className="min-w-0 flex-1">
              {!isPanelInteractive ? (
                <div
                  className={`content-status-indicator flex min-w-0 flex-1 items-center gap-1 ${
                    isLight ? 'text-gray-600' : 'text-gray-400'
                  }`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="relative overflow-hidden">
                      <span className="fade-text block whitespace-nowrap">
                        {currentTabId ? (
                          <>
                            {getCurrentTabTitle()}
                ...
                <div
                  className={`content-status-indicator content-refreshing flex min-w-0 flex-1 items-center gap-1 ${
            ...
            /* Web: no browser-tab pipeline — keep flex space so action buttons stay right-aligned */
        ...
        <div className="flex flex-shrink-0 items-center gap-1">
          {/* Analyze current tab — extension only (no active tab on standalone web) */}
          {isExtensionContext() && (
            <button
              onClick={onRefreshClick}
              disabled={isContentFetching}
              className={`relative flex h-[26px] w-[26px] items-center justify-center rounded-md transition-colors ${
                isContentFetching
          ...
          {moreOptionsMenu && (
            <DropdownMenu
              ...
              <DropdownMenuItem
                onClick={moreOptionsMenu.onCloseSession}
                shortcut="⌘ C"
                isLight={moreOptionsMenu.isLight}>
                Close Chat
              </DropdownMenuItem>
              <DropdownMenuItem onClick={moreOptionsMenu.onClearAllMessages} isLight={moreOptionsMenu.isLight}>
                Clear All Chat Messages
              </DropdownMenuItem>
              <DropdownMenuItem onClick={moreOptionsMenu.onClearAllSessions} isLight={moreOptionsMenu.isLight}>
                Clear All Chats
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownAccordion label="Export Chat" isLight={moreOptionsMenu.isLight}>
                <DropdownMenuItem
                  ...
                  onClick={e => {
                    e.preventDefault();
                    e.stopPropagation();
                    moreOptionsMenu.onExportAsMarkdown();
                  }}
                  isLight={moreOptionsMenu.isLight}>
                  Export as Markdown
                </DropdownMenuItem>
                <DropdownMenuItem
                  ...
                  onClick={e => {
                    e.preventDefault();
                    e.stopPropagation();
                    moreOptionsMenu.onExportAsHTML();
                  }}
                  isLight={moreOptionsMenu.isLight}>
                  Export as HTML
                </DropdownMenuItem>
                  ...
                  {showCopyFeedback ? (
                    <svg
                      className={cn(
                        ...
                        moreOptionsMenu.isLight ? 'text-green-600' : 'text-green-400',
                      )}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    ...
                    <svg
                      className={cn(
                        'h-4 w-4 flex-shrink-0 opacity-60 transition-all duration-200',
                        moreOptionsMenu.isLight ? 'text-gray-500' : 'text-gray-400',
                      )}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={moreOptionsMenu.onOpenAbout} isLight={moreOptionsMenu.isLight}>
                About Project Hands-Off
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={moreOptionsMenu.onClose} isLight={moreOptionsMenu.isLight}>
                Close Panel
              </DropdownMenuItem>
            </DropdownMenu>
          )}
