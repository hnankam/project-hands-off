  ...
  if (content.allDOMContent) {
    ...
    if (content.allDOMContent.allFormData) {
      ...
      if (content.allDOMContent.allFormData.length > 0) {
        debug.log(
          '     First 10 elements:',
          content.allDOMContent.allFormData.slice(0, 10).map((f: any) => ({
    ...
    if (content.allDOMContent.clickableElements) {
      ...
      debug.log(
        `   - clickableElements: ${clickableSize} KB (${content.allDOMContent.clickableElements.length} elements)`,
      );
      if (content.allDOMContent.clickableElements.length > 0) {
        debug.log(
          '     First 10 elements:',
          content.allDOMContent.clickableElements.slice(0, 10).map((c: any) => ({
 ...
 */
const updateCache = (cache: Map<string, CacheEntry>, key: string, content: any, timestamp: number, tabId: number) => {
  // Update cache
...
export const useContentManager = ({
   ...
   */
  const fetchFreshPageContent = useCallback(
    async (force = false, tabIdOverride?: number) => {
      ...
      if (!FEATURES.extensionMessaging()) {
        debug.log('[ContentManager] Skipping fetch — extension messaging not available (web mode)');
        return;
      }
  ...
  ); // Reduced dependencies using refs
   ...
   */
  const handleContentBecameStale = useCallback(
    (message: any) => {
    ...
    [isPanelActive, isAgentLoading, fetchFreshPageContent],
  ); // Stable
   ...
   */
  useEffect(() => {
    if (!FEATURES.extensionMessaging() || !isActive) return;
