...
import { cn, ErrorDisplay, LoadingSpinner, Button, DropdownMenu, DropdownMenuItem, DropdownMenuSeparator, SessionList, SessionTabs, DropdownSubmenu } from '@extension/ui';
...
const SidePanel = () => {
  ...
  const handleExportSessionAsPDF = () => {
    ...
    if (!doc) return;
    doc.document.write(`<!doctype html><html><head><meta charset="utf-8"/><title>${current?.title || 'Session PDF'}</title><style>body{font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,sans-serif;padding:24px}h1{margin-top:0}pre{white-space:pre-wrap;word-break:break-word}</style></head><body><h1>${current?.title || 'Session'}</h1><pre>${JSON.stringify(messages, null, 2)}</pre></body></html>`);
    doc.document.close();
    // Give the new window a moment to render, then open print dialog
    setTimeout(() => {
      try { doc.focus(); doc.print(); } catch {}
    }, 200);
  };
        ...
        <div className={cn(
          ...
          <div className="flex items-center space-x-1 flex-shrink-0">
            ...
            >
              ...
              <DropdownMenuItem onClick={openClearSessionsConfirm}>
                Clear All Sessions
              </DropdownMenuItem>
                ...
                <div className="px-3 py-4 space-y-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      >
                        All <strong>{messageCount}</strong> {messageCount === 1 ? 'message' : 'messages'} from "{sessions.find(s => s.id === currentSessionId)?.title || 'this session'}" will be permanently deleted from storage and cannot be recovered.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <>
            ...
            <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4">
              ...
              >
                ...
                {/* Content */}
                <div className="px-3 py-5 space-y-3 text-center">
                  ...
                  <div>
                    <pre className={cn('text-xs whitespace-pre-wrap break-words', isLight ? 'text-gray-800' : 'text-gray-200')}>
                      {aboutText}
                    </pre>
                    <div className="mt-2">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className={cn('text-xs underline break-all', isLight ? 'text-blue-600 hover:text-blue-700' : 'text-blue-400 hover:text-blue-300')}
                      >
