...
generic_agent = LlmAgent(
        ...
        planner=BuiltInPlanner(
            thinking_config=types.ThinkingConfig(
                include_thoughts=False, # when enabled, coPilotActions with handle fail. Only render actions work
...
de_agent = LlmAgent(
        name="de_agent",
        # model=LiteLlm(model="bedrock/us.anthropic.claude-3-5-sonnet-20241022-v2:0"), 
        model=LiteLlm(model="bedrock/us.anthropic.claude-sonnet-4-5-20250929-v1:0"), 
        # model=LiteLlm(model="databricks/databricks-claude-opus-4-1"), 
        # model=LiteLlm(model="vertex_ai/claude-sonnet-4-5@20250929"), 

        # planner=PlanReActPlanner(),
        # planner=BuiltInPlanner(
        #     thinking_config=types.ThinkingConfig(
        #         include_thoughts=True, # when enabled, coPilotActions with handle fail. Only render actions work
        #         thinking_budget=1024,
        #     )
        # ),
       instruction="""
...
adk_generic_agent = ADKAgent(
    adk_agent=generic_agent,
    app_name="project_hands_off",
    user_id="demo_user",
    session_timeout_seconds=3600,
    use_in_memory_services=True
)

adk_de_agent = ADKAgent(
    adk_agent=de_agent,
    app_name="project_hands_off",
    user_id="demo_user",
    session_timeout_seconds=3600,
...
# Create FastAPI app
app = FastAPI(title="ADK Middleware Project Hands Off")

root_agent = de_agent

# Add the ADK endpoint
add_adk_fastapi_endpoint(app, adk_generic_agent, path="/generic_agent")
add_adk_fastapi_endpoint(app, adk_de_agent, path="/de_agent")
