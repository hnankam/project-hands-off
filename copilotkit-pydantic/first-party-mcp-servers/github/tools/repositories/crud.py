...
def list_repositories(
    ...
    try:
        ...
        return ListRepositoriesResponse(
            ...
            per_page=per_page,
            has_next_page=False,
            error_message=f"Failed to list repositories: {str(e)}",
        )
...
def get_repository(
    ...
    """
    Get complete information about a specific repository.
    ...
    try:
        ...
        return GetRepositoryResponse(
            repository=None,
            error_message=f"Failed to get repository {repo}: {str(e)}",
        )
...
def create_repository(
    ...
    """
    Create a new repository for the authenticated user or specified organization.
    ...
    Returns:
        CreateRepositoryResponse with created repository details
    """
    try:
        client = get_github_client(token_credential_key, base_url_credential_key)
        ...
        return CreateRepositoryResponse(
            repository=None,
            message=None,
            error_message=f"Failed to create repository {name}: {str(e)}",
        )
...
def delete_repository(
    ...
    **WARNING:** This operation is irreversible!
    ...
    Args:
        token_credential_key: Credential key for GitHub Personal Access Token (requires delete_repo scope)
        repo: Repository name in format "owner/repo"
        base_url_credential_key: Credential key for GitHub API base URL (optional, defaults to public GitHub)

    Returns:
        DeleteRepositoryResponse with confirmation message
    """
    try:
        client = get_github_client(token_credential_key, base_url_credential_key)
        repo_obj = client.get_repo(repo)
        ...
        return DeleteRepositoryResponse(
            repo_name=repo,
            message=None,
            error_message=f"Failed to delete repository {repo}: {str(e)}",
        )
...
def fork_repository(
    ...
    """
    Create a fork of the specified repository.
    ...
    Returns:
        ForkRepositoryResponse with forked repository details
    """
    try:
        client = get_github_client(token_credential_key, base_url_credential_key)
        repo_obj = client.get_repo(repo)
        ...
        return ForkRepositoryResponse(
            repository=None,
            message=None,
            error_message=f"Failed to fork repository {repo}: {str(e)}",
        )
