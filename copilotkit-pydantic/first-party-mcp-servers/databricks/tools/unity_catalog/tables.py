...
def list_tables(
    ...
    Args:
        ...
        limit: Number of tables to return in a single request. Must be positive integer. Default: 25
            Note: When show_expanded=True, limit is automatically capped at 10 to prevent excessively large responses
        page: Zero-indexed page number for pagination. Default: 0
        show_expanded: Boolean flag to control response detail level. Default: False
            - False (default): Returns only essential fields (name, type, owner, timestamps) - faster, smaller response
            - True: Returns all fields including columns, properties, constraints, and advanced metadata (max 10 tables per page)
    ...
    Returns:
        ...
        - tables: List of TableInfoModel objects with table metadata
            - Essential fields (always included): name, full_name, catalog_name, schema_name, table_type, 
              owner, comment, created_at, updated_at, table_id
            - Expanded fields (only when show_expanded=True): columns, properties, storage_location, 
              data_source_format, view_definition, constraints, and all advanced metadata
    ...
    Note:
        ...
        - Use list_table_summaries() for even more lightweight discovery (name and type only)
        - Use show_expanded=False for faster responses when detailed metadata isn't needed
    """
    try:

        client = get_workspace_client(host_credential_key, token_credential_key)
        
        # Cap limit at 10 when show_expanded=True to prevent excessively large responses
        effective_limit = min(limit, 10) if show_expanded else limit
        ...
        for table in tables_iterator:
            if show_expanded:
                # Full metadata including columns and all advanced fields
                columns = None
                if table.columns:
                    columns = [
                        ColumnInfoModel(
                ...
                # Essential fields only - use compact model for smaller, faster response
                tables_list.append(
                    TableInfoCompactModel(
                        name=table.name,
                        full_name=table.full_name,
