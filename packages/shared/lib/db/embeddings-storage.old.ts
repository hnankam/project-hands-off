  ...
  async storeHTMLChunks(data: {
    ...
    try {
      ...
      for (let i = 0; i < data.chunks.length; i += BATCH_SIZE) {
        ...
        console.log(`[EmbeddingsStorage] ⏳ Progress: ${Math.min(i + BATCH_SIZE, data.chunks.length)}/${data.chunks.length}`);
      }

      console.log(`[EmbeddingsStorage] ✅ Stored ${data.chunks.length} HTML chunks with HNSW indexes (batched with yielding)`);
    } catch (error) {
